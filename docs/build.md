...
/mnt/d/e996_navmesh/Bin/lib/libnavigation.so
```
...
对应逻辑见 [CMakeLists.txt](/d:/e996_navmesh/CMakeLists.txt#L84)。
...
当前 [CMakeLists.txt](/d:/e996_navmesh/CMakeLists.txt#L4) 已经改成“未显式指定时默认使用 `Debug`”。另外，`Debug` 配置还会额外附加 `-O0 -g`，让断点和单步更稳定。
...
这个行为来自 [CMakeLists.txt](/d:/e996_navmesh/CMakeLists.txt#L20)：
...
-- Configuring done
-- Generating done
-- Build files have been written to: /mnt/d/e996_navmesh/build
```
...
```text
/usr/bin/ld: cannot open output file /mnt/d/e996_navmesh/Bin/lib/libnavigation.so: Read-only file system
```
...
- 确认当前仓库目录可写
- 或者修改 [CMakeLists.txt](/d:/e996_navmesh/CMakeLists.txt#L20)，把库输出目录改到其他可写位置
